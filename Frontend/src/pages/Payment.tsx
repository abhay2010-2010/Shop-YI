...
const CardChip: React.FC = () => (
  <svg className="w-8 h-8 sm:w-10 sm:h-10 text-yellow-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
const VisaLogo: React.FC = () => (
  <svg className="w-12 h-8 sm:w-16 sm:h-10" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" strokeWidth="1.5">
...
const CustomToast: React.FC<CustomToastProps> = ({ message, onClose }) => {
  ...
  const base = "fixed top-5 left-1/2 transform -translate-x-1/2 px-4 py-3 rounded-lg shadow-lg z-50 flex items-center space-x-3 text-white";
  const colors =
...
function Payment() {
  ...
  const handleMonthChange = (e: React.ChangeEvent<HTMLSelectElement>) => setExpMonth(e.target.value);
  const handleYearChange = (e: React.ChangeEvent<HTMLSelectElement>) => setExpYear(e.target.value.substring(2));
  ...
  const handlePayment = useCallback(
    (e: React.FormEvent) => {
      e.preventDefault();
      if (cardNumber.length < 16 || cardHolder === "FULL NAME" || expMonth === "MM" || expYear === "YY" || cvv.length < 3) {
        showMessage("error", "Invalid Input", "Please fill all card details correctly.");
        return;
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-slate-100 p-4 sm:p-6 md:p-8">
      ...
      <div className="flex flex-col md:flex-row w-full max-w-5xl bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        {/* Right - Payment Form */}
        <form onSubmit={handlePayment} className="md:w-1/2 p-6 sm:p-8 space-y-5">
